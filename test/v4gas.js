...
describe("Mint Phase", () => {
  beforeEach(async function () {
    // Get the ContractFactory and Signers here.
    [owner, addr1, addr2, ...addrs] = await ethers.getSigners();
    Ticket = await ethers.getContractFactory("TicketV4", owner);
  ...
  it("Check if user can mint", async () => {
    ...
    // });
    // wei = gas.toNumber() * 100 * 1000000000;
    // table.push([1000, gas.toNumber(), ethers.utils.formatEther(wei.toString())]);
    console.log(table.toString());
    const balance = await contract.balanceOf(addr1.address, TICKET);
    assert(balance.eq(BigNumber.from(1111)));
  });
});
